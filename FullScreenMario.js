...
var FullScreenMario = (function (GameStartr) {
  ...
  // Dictionary for counting all user behaviors
  var behaviorDict = {
    left: 0,
    right: 0,
    up: 0,
  ...
  function resetStatsHolder(EightBitter, customs) {
    ...
    if (customs.width < 560) {
      EightBitter.StatsHolder.getContainer().children[0].cells[4].style.display =
        "none";
    }
  }
  ...
  function gameStart() {
    ...
    EightBitter.StatsHolder.set(
      "lives",
      EightBitter.settings.statistics.values.lives.valueDefault
    );
  ...
  function addPlayer(left, bottom) {
    ...
    if (EightBitter.MapScreener.underwater) {
      ...
      EightBitter.TimeHandler.addClassCycle(
        player,
        ["swim1", "swim2"],
        "swimming",
        5
  ...
  function keyDownPause(player, event) {
    if (!player.EightBitter.GamesRunner.getPaused()) {
      player.EightBitter.TimeHandler.addEvent(
        player.EightBitter.GamesRunner.pause,
        7,
        true
      );
    }
  ...
  function maintainCharacters(EightBitter, characters) {
    ...
    for (i = 0; i < characters.length; ++i) {
      ...
      if (character.resting) {
        ...
        if (!character.nofall) {
          character.yvel +=
            character.gravity || EightBitter.MapScreener.gravity;
        }
        character.yvel = Math.min(
          character.yvel,
          EightBitter.MapScreener.maxyvel
        );
      }
  ...
  function maintainPlayer(EightBitter) {
    ...
    if (player.xvel > 0) {
      if (player.right > EightBitter.MapScreener.middleX) {
        ...
        if (
          player.right >
          EightBitter.MapScreener.right - EightBitter.MapScreener.left
        ) {
  ...
  function isThingOnThing(thing, other) {
    ...
    if (thing.player && thing.bottom < other.bottom && other.type === "enemy") {
      return true;
    }
    ...
    if (
      thing.bottom <=
      other.top + other.toly + Math.abs(thing.yvel - other.yvel)
    ) {
      return true;
    }
  ...
  function playerStarUp(thing, timeout) {
    ...
    thing.EightBitter.AudioPlayer.addEventListener(
      "Powerup",
      "ended",
      thing.EightBitter.AudioPlayer.playTheme.bind(undefined, "Star", true)
    );
  ...
  function spawnDeadGoomba(thing) {
    thing.EightBitter.TimeHandler.addEvent(
      FullScreenMario.prototype.killNormal,
      21,
      thing
    );
  }
  ...
  function spawnLakitu(thing) {
    if (thing.EightBitter.isThingAlive(thing.EightBitter.MapScreener.lakitu)) {
    ...
    thing.EightBitter.TimeHandler.addEventInterval(
      thing.EightBitter.animateLakituThrowingSpiny,
      140,
      Infinity,
      thing
    );
  ...
  function spawnRandomCheep(EightBitter) {
    ...
    spawn = EightBitter.ObjectMaker.make("CheepCheep", {
      flying: true,
      xvel: EightBitter.NumberMaker.random() * EightBitter.unitsize * 1.4,
      yvel: EightBitter.unitsize * -1.4,
    });

    EightBitter.addThing(
      spawn,
      EightBitter.NumberMaker.random() * EightBitter.MapScreener.width,
  ...
  function spawnCustomText(thing) {
    var top = thing.top,
      ...
      spacingVerticalBlank =
        thing.spacingVerticalBlank * thing.EightBitter.unitsize,
  ...
  function activateWindowDetector(thing) {
    if (
      thing.EightBitter.MapScreener.right - thing.EightBitter.MapScreener.left <
      thing.left
    ) {
      return;
    }
  ...
  function activateSectionBefore(thing) {
    ...
    command = {
      thing: "DetectWindow",
      x: left + (before ? section.before.width : 0),
      y: 0,
  ...
  function collideShell(thing, other) {
    ...
    if (other.xvel) {
      ...
      thing.EightBitter.AudioPlayer.play("Kick");
      thing.EightBitter.scoreOn(
        thing.EightBitter.findScore(other.enemyhitcount),
        thing
      );
  ...
  function collideShellPlayer(thing, other) {
    ...
    // the shell isn't moving away
      ...
      if (
        !other.hitcount &&
        ((shelltoleft && other.xvel > 0) || (!shelltoleft && other.xvel < 0))
      ) {
        thing.death(thing);
      }
    }
  ...
  function collideEnemy(thing, other) {
    ...
    if (
      ...
      // A non-star player kills the enemy with spawn, and hops
        ...
        thing.EightBitter.setBottom(
          thing,
          Math.min(thing.bottom, other.top + thing.EightBitter.unitsize)
        );
        ...
        thing.EightBitter.removeClasses(
          thing,
          "running skidding jumping one two three"
        );
        thing.hopping = true;
  ...
  function collideBottomBlock(thing, other) {
    ...
    selectedData.block += 1;
    if (thing.used) {
      thing.EightBitter.AudioPlayer.play("Bump");
      return;
    }
    if (thing.hidden) selectedData.blockHidden += 1;
    thing.used = true;
    thing.hidden = false;
    thing.up = other;
    ...
    thing.EightBitter.TimeHandler.addEvent(
      thing.EightBitter.animateSolidContents,
      7,
      thing,
      other
    );
  ...
  function collideFlagpole(thing, other) {
    ...
    var height = (other.bottom - thing.bottom) | 0,
      scoreAmount = scorePlayerFlag(thing, height / thing.EightBitter.unitsize),
      scoreThing = thing.EightBitter.ObjectMaker.make("Text" + scoreAmount);
    ...
    thing.EightBitter.setRight(
      thing,
      other.left + thing.EightBitter.unitsize * 3
    );
    thing.EightBitter.killNormal(other);
    ...
    thing.EightBitter.TimeHandler.addEvent(
      thing.EightBitter.StatsHolder.increase.bind(
        thing.EightBitter.StatsHolder
      ),
      72,
      "score",
      scoreAmount
    ...
    thing.EightBitter.TimeHandler.addEventInterval(
      function () {
        ...
        if (thing.EightBitter.StatsHolder.get("time") <= 0) {
          thing.EightBitter.TimeHandler.addEvent(function () {
            thing.EightBitter.animateEndLevelFireworks(
              thing,
              other,
              numFireworks
            );
  ...
  function moveSmart(thing) {
    ...
    if (
      !thing.resting ||
      !thing.EightBitter.isCharacterOnResting(thing, thing.resting)
    ) {
      if (thing.moveleft) {
  ...
  function moveBowserFire(thing) {
    ...
    thing.EightBitter.shiftVert(
      thing,
      Math.min(
        Math.max(0, thing.ylev - thing.bottom),
        thing.EightBitter.unitsize
      )
    );
  }
  ...
  function moveFloating(thing) {
    ...
    if (thing.top <= thing.end) {
      thing.yvel = Math.min(
        thing.yvel + thing.EightBitter.unitsize / 64,
        thing.maxvel
      );
    }
  ...
  function moveSliding(thing) {
    ...
    if (thing.EightBitter.MapScreener.left + thing.left <= thing.begin) {
      thing.xvel = Math.min(
        thing.xvel + thing.EightBitter.unitsize / 64,
        thing.maxvel
      );
    }
  ...
  function movePlatform(thing) {
    ...
    if (
      thing === thing.EightBitter.player.resting &&
      thing.EightBitter.player.alive
    ) {
  ...
  function movePlatformSpawn(thing) {
    if (thing.bottom < 0) {
      thing.EightBitter.setTop(
        thing,
        thing.EightBitter.MapScreener.bottomPlatformMax
      );
  ...
  function moveFalling(thing) {
    ...
    thing.EightBitter.shiftVert(
      thing,
      (thing.yvel += thing.EightBitter.unitsize / 8)
    );
    thing.EightBitter.setBottom(thing.EightBitter.player, thing.top);
  ...
  function moveFreeFalling(thing) {
    ...
    if (
      thing.yvel >= (thing.fallThresholdEnd || thing.EightBitter.unitsize * 2)
    ) {
  ...
  function movePlatformScale(thing) {
    ...
    // If this still has velocity from a player, stop or fall less
      ...
      if (!thing.partners) {
        ...
        thing.yvel = Math.max(thing.yvel - thing.EightBitter.unitsize / 16, 0);
      }
    }
    ...
    thing.EightBitter.shiftVert(thing.partners.partnerPlatform, -thing.yvel);

    // This platform's string grows with its yvel
    thing.EightBitter.setHeight(
      thing.partners.ownString,
      thing.partners.ownString.height + thing.yvel / thing.EightBitter.unitsize
    );
  ...
  function moveBubble(thing) {
    if (
      thing.top <
      thing.EightBitter.MapScreener.top + thing.EightBitter.unitsize * 16
    ) {
      thing.EightBitter.killNormal(thing);
    }
  }
  ...
  function movePlayer(thing) {
    ...
    // Jumping
      ...
      if (!thing.EightBitter.MapScreener.underwater) {
        ...
        var dy =
          FullScreenMario.unitsize /
          Math.pow(
            thing.keys.jumplev,
            thing.EightBitter.MapScreener.jumpmod - 0.0014 * thing.xvel
          );
        thing.yvel = Math.max(
          thing.yvel - dy,
          thing.EightBitter.MapScreener.maxyvelinv
        );
      }
    }
    ...
    if (thing.keys.run != 0 && !thing.crouching) {
      var dir = thing.keys.run,
        ...
        sprinting =
          (thing.keys.sprint && !thing.EightBitter.MapScreener.underwater) || 0,
        adder = dir * (0.098 * (Number(sprinting) + 1));
    ...
    if (Math.abs(thing.xvel) < 0.14) {
      if (thing.running) {
        ...
        thing.EightBitter.removeClasses(
          thing,
          "running skidding one two three"
        );
    ...
    if (thing.xvel > 0) {
      ...
      if (
        thing.moveleft &&
        (thing.resting || thing.EightBitter.MapScreener.underwater)
      ) {
        thing.EightBitter.unflipHoriz(thing);
        thing.moveleft = false;
      }
    ...
    if (thing.resting) {
      ...
      if (thing.jumping) {
        ...
        thing.EightBitter.addClass(
          thing,
          Math.abs(thing.xvel) < 0.14 ? "still" : "running"
        );
      }
      // Paddling
      if (thing.paddling) {
  ...
  function movePlayerVine(thing) {
    ...
    if (thing.keys.run !== 0 && thing.keys.run === thing.attachedDirection) {
      ...
      if (thing.attachedDirection === -1) {
        thing.EightBitter.setRight(
          thing,
          attachedSolid.left - thing.EightBitter.unitsize
        );
      }
      // Leaving to the right
        ...
        thing.EightBitter.setLeft(
          thing,
          attachedSolid.right + thing.EightBitter.unitsize
        );
      }
    ...
    if (
      thing.bottom <
      thing.EightBitter.MapScreener.top - thing.EightBitter.unitsize * 4
    ) {
  ...
  function animateEndLevelFireworks(thing, other, numFireworks) {
    ...
    thing.EightBitter.arrayToBeginning(
      flag,
      thing.EightBitter.GroupHolder.getGroup(flag.groupType)
    );

    thing.EightBitter.TimeHandler.addEventInterval(
      function () {
        thing.EightBitter.shiftVert(flag, thing.EightBitter.unitsize * -0.25);
      },
      1,
      flagMovements
    );
  ...
  function animatePlayerFire(thing) {
    ...
    thing.EightBitter.addThing(
      ball,
      xloc,
      thing.top + thing.EightBitter.unitsize * 8
    );
    ball.animate(ball);
  ...
  function animatePlayerPaddling(thing) {
    if (!thing.paddlingCycle) {
      thing.EightBitter.removeClasses(
        thing,
        "skidding paddle1 paddle2 paddle3 paddle4 paddle5"
      );
    ...
    thing.yvel = thing.EightBitter.unitsize * -0.84;
  }
  ...
  function animatePlayerRunningCycle(thing) {
    ...
    thing.running = thing.EightBitter.TimeHandler.addClassCycle(
      thing,
      ["one", "two", "three", "two"],
      "running",
      function (event) {
        event.timeout =
          5 + Math.ceil(thing.maxspeedsave - Math.abs(thing.xvel));
      }
    );
  }
  ...
  function killPlayer(thing, big) {
    ...
    // Regular big: regular (enemy, time, etc.) kill
      ...
      // The player can't survive this: animate a death
        ...
        EightBitter.arrayToEnd(
          thing,
          EightBitter.GroupHolder.getGroup(thing.groupType)
        );
    ...
    if (EightBitter.StatsHolder.get("lives") > 0) {
      EightBitter.TimeHandler.addEvent(
        area.onPlayerDeath.bind(EightBitter),
        area.onPlayerDeathTimeout,
        EightBitter
      );
    } else {
      EightBitter.TimeHandler.addEvent(
        area.onGameOver.bind(EightBitter),
  ...
  function mapEntranceNormal(EightBitter, location) {
    ...
    EightBitter.addPlayer(EightBitter.unitsize * 16, EightBitter.unitsize * 16);
  }
  ...
  function mapEntranceCastle(EightBitter) {
    EightBitter.addPlayer(EightBitter.unitsize * 2, EightBitter.unitsize * 56);
  }
    ...
    EightBitter.TimeHandler.addEventInterval(
      function () {
        ...
        if (player.top < threshold) {
          EightBitter.TimeHandler.addEvent(
            EightBitter.animatePlayerOffVine,
            49,
            player
          );
          return true;
  ...
  function mapExitPipeVertical(thing, other) {
    if (
      !thing.resting ||
      typeof other.transport === "undefined" ||
  ...
  function mapAddStretched(prething) {
    var EightBitter = EightBittr.prototype.ensureCorrectCaller(this),
      ...
      y = (EightBitter.MapScreener.floor - prething.y) * EightBitter.unitsize,
      thing = EightBitter.ObjectMaker.make(prething.thing, {
        width: boundaries.right - boundaries.left,
        height: prething.height || EightBitter.getAbsoluteHeight(prething.y),
      });
  ...
  function macroScale(reference, prethings, area, map, scope) {
    var x = reference.x || 0,
      ...
      collectionName =
        "ScaleCollection--" +
        [x, y, widthLeft, widthRight, dropLeft, dropRight].join(","),
  ...
  function macroPlatformGenerator(reference, prethings, area, map, scope) {
    var output = [],
      ...
      x = reference.x || 0,
      yvel = direction * scope.unitsize * 0.42,
      i;
